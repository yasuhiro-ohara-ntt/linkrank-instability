...
start_short = strftime("%Y-%m-%d-%H%M%S", gmtime(start))
end_short = strftime("%Y-%m-%d-%H%M%S", gmtime(end))

# range = str(start) + '-' + str(end)
range = start_short + '-' + end_short
...
while current <= end:
    ...
    while(stream.get_next_record(rec)):
        ...
        while(elem):
            ...
            while i + 1 < len(ases):
                ...
                link_dir = (ases[i], ases[i + 1])
                if (ases[i] < ases[i + 1]):
                    link_ind = (ases[i], ases[i + 1])
                else:
                    link_ind = (ases[i + 1], ases[i])
                print >> f, link_dir, link_ind
                linkset.add(link_dir)
                linkset_indirect.add(link_ind)
                i += 1
            elem = rec.get_next_elem()
            count += 1
            print '\r',
    ...
    if (len(list_linkset) > 6):
        i = 7
        past6events = {}
        while (i > 1):
            downs = list_linkset[-1 * i] - list_linkset[-1 * i + 1]
            ups = list_linkset[-1 * i + 1] - list_linkset[-1 * i]
            print "past6:", -1 * i, -1 * i + 1,
            print len(downs), len(ups)
            for link in downs:
                if (link in past6events):
                    past6events[link] += 1
                else:
                    past6events[link] = 1
                print "past6downs: ", link, past6events[link]
            for link in ups:
                if (link in past6events):
                    past6events[link] += 1
                else:
                    past6events[link] = 1
                print "past6ups: ", link, past6events[link]
            i -= 1
        past6linkranks = {}
        for k, v in sorted(past6events.items(), key=lambda x:x[1]):
            if (v in past6linkranks):
                past6linkranks[v] += 1
            else:
                past6linkranks[v] = 1
        print "past6: ",
        print >> f, "past6: ",
        for k, v in sorted(past6linkranks.items()):
            print "[", k, "]:", v, " ",
            print >> f, "[", k, "]:", v, " ",
        print ""
        print >> f, ""
